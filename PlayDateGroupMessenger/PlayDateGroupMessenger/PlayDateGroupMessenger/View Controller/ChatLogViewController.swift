...
class ChatLogViewController: UICollectionViewController, UITextFieldDelegate, UICollectionViewDelegateFlowLayout, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    fileprivate func uploadToFirebaseStorageUsingImage(_ image: UIImage, completion: @escaping (_ imageUrl: String) -> ()) {
        ...
        if let uploadData = UIImageJPEGRepresentation(image, 0.2) {
            ...
            ref.putData(uploadData, metadata: nil, completion: { (metadate, error) in
                
                if error != nil {
                    print("Failed to upload image:", error!)
                    return
                }
                if let imageUrl = metadate?.downloadURL()?.absoluteString {
                    completion(imageUrl)
                }
    ...
    fileprivate func setupCell(_ cell: ChatMessageCollectionViewCell, message: Message) {
